...
import SalesChallanSelect from "../../challan/ExportComponent";
import { createNewInvoice, getChallanForInvoice, getInvoiceNo } from "./action";
import { newInvoiceSchema } from "./type";
import SelectCustomers from "../../customer/ExportComponent";
    ...
    () => getChallanForInvoice(Number(input.challanId)),
    {
      onSuccess: (data: any) => {
        ...
        setInput((prev) => ({
          ...
          exclusiveContact: data.contactPerson,
          remarks: data.remarks,
          products: data.SalesChallanItems.map((item: any) => ({
            group: item.product.group.name,
            groupId: item.product.group.id,
            item: item.product.id,
            quantity: item.quantity,
            rate: 0, // Challan may not have rate info
            discount: "0",
            amount: 0, // Will be calculated when rate is entered
            batchNo: "",
  ...
  useEffect(() => {
    if (initialData) {
      setInput({
        ...
        exclusiveContact: initialData.exclusiveContact || "",
  ...
  return (
    <>
      <div className="mb-6 space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="space-y-2">
            <label className="text-sm font-medium">Sales Challan</label>
            <SalesChallanSelect
        ...
        <InputWithLabel
          ...
          value={input.exclusiveContact}
          onChange={(e) => handleInputChange("exclusiveContact", e.target.value)}
        />
      ...
      <div className="mt-6 flex justify-end gap-2">
        <Button
          variant="outline"
          onClick={() => router.push("/admin/sales/invoice")}
        >
          Cancel
        </Button>
        <LoadingButton
